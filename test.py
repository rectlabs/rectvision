from src.rectvision.data.converters.rectvisConverter import *
from src.rectvision import models

token = ""
rv = RectvisionConverter(0.5, 0.3, 0.2, 'yolo-txt', token = token)

project_dir = "/"
num_classes, img_size, batch_size, num_epochs, labels, project_name, project_dir = 2, 512, 2, 2, ["mask", "no_mask"], "mask_detect", project_dir
model_yolo = models.Yolov5(num_classes, img_size, batch_size, num_epochs, labels, project_name, project_dir)

# training
model_yolo.train()

# get map
model_yolo.get_map()

# run inference
model_yolo.inference(images='dataset/test/images', confidence=0.0, out_dir='detections/')
